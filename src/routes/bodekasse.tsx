...
function BodekassePage() {
  ...
  const add = async (e: React.FormEvent) => {
    ...
    if (reason.length > 200) return toast.error("Reason too long");
    const { error } = await supabase.from("fines").insert({
      player_id: playerId, amount: amt, reason: reason.trim(),
    });
    if (error) toast.error(error.message);
    else { setAmount(""); setReason(""); toast.success("Bøde added 💸"); load(); }
  };
  ...
  return (
    <div className="space-y-8">
      <section className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-primary mb-2">Team kitty</p>
        ...
        <div className="rounded-xl border border-primary/40 bg-card px-6 py-4 shadow-[var(--shadow-glow)]">
          <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
            <PiggyBank className="h-4 w-4" /> Total pot
          </div>
      ...
      <form onSubmit={add} className="grid md:grid-cols-[1fr_140px_1fr_auto] gap-2 items-start rounded-xl border border-border bg-card p-4">
        <Select value={playerId} onValueChange={setPlayerId}>
          <SelectTrigger className="bg-input border-border"><SelectValue placeholder="Player" /></SelectTrigger>
          <SelectContent>
            {players.map((p) => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}
          </SelectContent>
        </Select>
        <Input type="number" min="1" placeholder="Amount" value={amount} onChange={(e) => setAmount(e.target.value)} className="bg-input border-border" />
        <Input placeholder="Reason (e.g. for sent for late)" value={reason} onChange={(e) => setReason(e.target.value)} maxLength={200} className="bg-input border-border" />
        <Button type="submit" className="gap-2"><Coins className="h-4 w-4" /> Add bøde</Button>
      </form>

      <section>
        <h2 className="font-display text-2xl mb-4">Owed per player</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {players.length === 0 && <p className="text-muted-foreground col-span-full">Add players in the Squad tab first.</p>}
        </div>
      </section>

      <section>
        <h2 className="font-display text-2xl mb-4">All bøder</h2>
        {fines.length === 0 ? (
          <p className="text-muted-foreground">No bøder yet — keep it that way 😉</p>
        ) : (
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            {fines.map((f, i) => (
              <div key={f.id} className={`flex items-center gap-4 px-5 py-3 ${i !== fines.length - 1 ? "border-b border-border/60" : ""}`}>
                ...
                <div className="text-xs text-muted-foreground hidden sm:block">{new Date(f.created_at).toLocaleDateString()}</div>
